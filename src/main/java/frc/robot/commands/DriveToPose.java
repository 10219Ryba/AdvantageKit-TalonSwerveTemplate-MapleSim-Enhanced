...
public class DriveToPose extends Command {
    ...
    private static final LoggedTunableNumber minThetaFFSReset = new LoggedTunableNumber("DriveToPose/Reset/MinThetaFF");
    ...
    private final PIDController driveController = new PIDController(0.0, 0.0, 0.0, Constants.loopPeriodSecs);
    ...
    public void execute() {
        ...
        // Update constraints
        // double extensionS = MathUtil.clamp(
        ...
        //                         MathUtil.interpolate(
        //                                 driveMaxAccelerationAuto.get(), driveMaxAccelerationAutoTop.get(),
        // extensionS))
        //                 : new TrapezoidProfile.Constraints(
        ...
        //         DriverStation.isAutonomous()
        //                 ? MathUtil.interpolate(thetaMaxVelocityAuto.get(), thetaMaxVelocityAutoTop.get(), extensionS)
        ...
        double linearFFScaler = MathUtil.clamp(
                (driveErrorAbs - linearFFMinRadius.get()) / (linearFFMaxRadius.get() - linearFFMinRadius.get()),
                0.0,
                1.0);
        ...
        double setpointVelocity = direction.norm()
        ...
        State driveSetpoint = driveProfile.calculate(
                ...
                new State(direction.norm(), -setpointVelocity), // Use negative as profile has zero at target
                new State(0.0, 0.0));
        ...
        double thetaSetpointVelocity = Math.abs((targetPose.getRotation().minus(lastGoalRotation)).getDegrees()) < 10.0
                ? (targetPose.getRotation().minus(lastGoalRotation)).getRadians() / (Timer.getTimestamp() - lastTime)
        ...
        thetaVelocity = MathUtil.interpolate(
                thetaVelocity, omegaFF.getAsDouble() * TunerConstants.kSpeedAt12Volts.in(MetersPerSecond), thetaS);
    ...
    private void resetProfile() {
        ...
        driveProfile = new TrapezoidProfile(
                DriverStation.isAutonomous()
                        ? new TrapezoidProfile.Constraints(driveMaxVelocityAuto.get(), driveMaxAccelerationAuto.get())
